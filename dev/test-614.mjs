...
console.log('[4] lyrics only accept the song that was asked for');
{
  ...
  ok(rank.includes("dScore >= 2 && exactTitle && verdict !== 'foreign'"), 'a duration-only match now also needs an exact title');
  ...
  ok(lyr.includes("_textOk(_lyrTitle, '') && _lyrArtistOk"), 'lyrist validates the reply before accepting it');
...
console.log('[6b2] MusicBrainz is asked both ways');
{
  ...
  ok(mb.includes('SC_MB_FETCH'), 'both reads carry an identifying User-Agent through one release-fetch budget (no wedging the run)');
}
